...
export const useCruisingMode = (carovanaId: string, username: string, carName?: string) => {
  ...
  const joinChannel = useCallback(async () => {
    try {
      ...
      peer.on('open', (id) => {
        console.log('[Cruising] Peer connesso con ID:', id);
        setIsActive(true);
        
        // Notifica presenza via Supabase Broadcast
    ...
    channel
      .on('broadcast', { event: 'unit_joined' }, ({ payload }) => {
        setUnits(prev => {
          ...
          // Se siamo noi i "vecchi", chiamiamo il nuovo arrivato
  ...
  return { isActive, isMuted, isSpeaking, units, joinChannel, leaveChannel, toggleMic, sendAlert };
};