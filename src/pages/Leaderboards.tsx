...
import { 
  Trophy, Sparkles, Heart, Loader2, ChevronLeft, Info, Medal, Users, ShieldCheck, User, ChevronRight
} from 'lucide-react';
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'reputation' ? (
                <motion.div key="rep-list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {topReputation?.map((u, i) => (
                    <div key={u.id} onClick={() => navigate(`/profile/${u.id}`)} className="bg-white/5 border border-white/5 p-5 rounded-[2rem] flex items-center justify-between group cursor-pointer hover:bg-white/10 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-white/10 bg-black">
                          {u.avatar_url ? <img src={u.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-zinc-800"><User size={24} /></div>}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-black italic uppercase text-white">@{u.username}</h4>
                            {u.is_admin && <ShieldCheck size={12} className="text-white" />}
                          </div>
                          <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Membro del Distretto</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className="text-xl font-black italic text-yellow-500 leading-none">{u.reputation || 0}</p>
