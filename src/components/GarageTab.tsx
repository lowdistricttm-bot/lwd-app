      ...
      <AnimatePresence>
        {isOwnProfile && isFormOpen && (
          <motion.div 
            ...
            className="bg-zinc-900/50 border border-white/10 p-6 overflow-hidden mb-8"
          >
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-zinc-500">{t.garage?.brand || "Marca"}</Label>
                  <Input required value={formData.brand} onChange={e => setFormData({...formData, brand: e.target.value})} className="bg-transparent border-zinc-800 rounded-none h-12 text-sm" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-zinc-500">{t.garage?.model || "Modello"}</Label>
                  <Input required value={formData.model} onChange={e => setFormData({...formData, model: e.target.value})} className="bg-transparent border-zinc-800 rounded-none h-12 text-sm" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-zinc-500">{t.garage?.year || "Anno"}</Label>
                  <Input value={formData.year} onChange={e => setFormData({...formData, year: e.target.value})} className="bg-transparent border-zinc-800 rounded-none h-12 text-sm" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-zinc-500">Targa</Label>
                  <Input value={formData.license_plate} onChange={e => setFormData({...formData, license_plate: e.target.value})} className="bg-transparent border-zinc-800 rounded-none h-12 text-sm" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-zinc-500">{t.garage?.suspension || "Assetto"}</Label>
                  <div className="grid grid-cols-2 gap-2">
                    {['STATIC', 'AIR'].map((type) => (
                      <button key={type} type="button" onClick={() => setFormData({...formData, suspension_type: type})} className={cn("h-12 border font-black uppercase italic text-xs tracking-widest transition-all", formData.suspension_type === type ? "bg-white border-white text-black" : "bg-transparent border-zinc-800 text-zinc-500 hover:border-zinc-600")}>{type}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-zinc-500">Foto Veicolo (Max 6)</Label>
                  <div onClick={() => fileInputRef.current?.click()} className="h-12 border border-dashed border-zinc-800 flex items-center justify-center cursor-pointer hover:border-white transition-colors bg-zinc-900/30">
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                  {existingImages.map((url, i) => (
                    <div key={`existing-${i}`} className="aspect-square relative bg-zinc-800 border border-white/5">
                      ...
                      <button type="button" onClick={() => removeExistingImage(i)} className="absolute -top-1 -right-1 w-5 h-5 bg-zinc-700 text-white flex items-center justify-center rounded-full"><X size={10} /></button>
                    </div>
                  ))}
                  {previews.map((url, i) => (
                    <div key={`new-${i}`} className="aspect-square relative bg-zinc-800 border border-white/5 opacity-70">
                      ...
                      <button type="button" onClick={() => removeNewPreview(i)} className="absolute -top-1 -right-1 w-5 h-5 bg-zinc-600 text-white flex items-center justify-center rounded-full"><X size={10} /></button>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <Textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="bg-transparent border-zinc-800 rounded-none min-h-[100px] text-sm" />
              </div>

              <div className="flex gap-4">
                <Button type="submit" disabled={addVehicle.isPending || updateVehicle.isPending} className="flex-1 bg-white/90 backdrop-blur-md text-black hover:bg-white rounded-none h-12 font-black uppercase italic tracking-widest transition-all">
                  {(addVehicle.isPending || updateVehicle.isPending) ? <Loader2 className="animate-spin" /> : editingId ? 'Aggiorna Veicolo' : 'Salva Veicolo'}
                </Button>
                <Button type="button" onClick={handleCloseForm} variant="outline" className="border-zinc-800 rounded-none h-12 font-black uppercase italic tracking-widest">Annulla</Button>
              </div>
            </form>
          </motion.div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {vehicles?.length === 0 ? (
          ...
          vehicles?.map((vehicle) => {
            ...
            return (
              <motion.div key={vehicle.id} layout className="bg-zinc-900/50 border border-white/5 overflow-hidden group">
                <div className="aspect-video bg-zinc-950 relative overflow-hidden">
                  ...
                  {isOwnProfile && (
                    <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all z-10">
                      ...
                      <button onClick={() => { if(confirm("Eliminare questo veicolo?")) deleteVehicle.mutate(vehicle.id); }} className="p-2 bg-black/60 text-white hover:bg-red-600 transition-colors"><Trash2 size={16} /></button>
                    </div>
                  )}
                  
                  <div className="absolute bottom-4 right-4 z-10">
                     <button onClick={(e) => { e.stopPropagation(); toggleLike.mutate(vehicle.id); }} className={cn("flex items-center gap-2 px-3 py-1.5 backdrop-blur-md border transition-all", vehicle.is_liked ? "bg-red-500 border-red-500 text-white" : "bg-black/40 border-white/10 text-white hover:bg-white/20")}>
