...
const StoryViewer = ({ allStories, initialUserIndex, onClose, currentUserId }: StoryViewerProps) => {
  ...
  const nextStory = useMemo(() => {
    if (currentIndex < userStories?.items.length - 1) return userStories.items[currentIndex + 1];
    if (userIndex < allStories.length - 1) return allStories[userIndex + 1].items[0];
    return null;
  ...
  useEffect(() => {
    ...
    if (currentStory?.music_metadata?.audio_url) {
      ...
      audio.loop = true; audio.preload = 'auto'; audio.volume = isMuted ? 0 : 0.5;
  ...
  const handleRemoveFromHighlight = async (e: React.MouseEvent) => {
    ...
    if (confirm("Vuoi rimuovere questa storia dai contenuti in evidenza?")) {
      try {
        await removeFromHighlight.mutateAsync({ highlightId: userStories.highlight_id, storyId: currentStory.id });
  ...
  return createPortal(
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] bg-black flex items-center justify-center overflow-hidden touch-none">
      <div className="relative w-full h-full md:h-[85vh] md:w-[420px] md:aspect-[9/16] bg-black md:rounded-[2.5rem] overflow-hidden flex flex-col shadow-2xl md:border md:border-white/10 z-10">
          ...
          <button onClick={handleProfileClick} className="flex items-center gap-3 group text-left">
            ...
            <div className="flex flex-col drop-shadow-md">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-white/60 uppercase tracking-widest">{getTimeAgo(currentStory.created_at)}</span>
              </div>
              <span className="text-[8px] font-bold text-white/80 uppercase tracking-widest">{roleLabel}</span>
              {currentStory.music_metadata && (
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-1.5 mt-1">
                  <MusicEqualizer />
                  <span className="text-[10px] font-bold text-white uppercase tracking-tight">{currentStory.music_metadata.artist} · {currentStory.music_metadata.title}</span>
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black z-10 overflow-hidden">
          <div className="absolute inset-0 z-0 pointer-events-none">
            <img src={currentStory.image_url} className="w-full h-full object-cover blur-[40px] opacity-40 scale-125" alt="Blurred background" />
            <div className="absolute inset-0 bg-black/40" />
          </div>

          <AnimatePresence mode="wait">
            <motion.div key={currentStory.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.2 }} className="w-full h-full flex items-center justify-center relative z-10">
              {isMediaLoading && <div className="absolute inset-0 flex items-center justify-center z-10"><Loader2 className="animate-spin text-white/40" size={40} /></div>}
              {isVideo ? (
                <video ref={videoRef} src={currentStory.image_url} className="w-full h-full object-contain" autoPlay playsInline preload="auto" muted={isMuted} onCanPlay={() => setIsMediaLoading(false)} onWaiting={() => setIsMediaLoading(true)} onPlaying={() => setIsMediaLoading(false)} onTimeUpdate={handleVideoTimeUpdate} onEnded={handleNext} onError={() => { setIsMediaLoading(false); handleNext(); }} />
              ) : (
                <img src={currentStory.image_url} className="w-full h-full object-contain" alt="Story" onLoad={() => setIsMediaLoading(false)} onError={() => { setIsMediaLoading(false); handleNext(); }} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
          ...
          <div className="px-4 flex w-full max-w-md mx-auto items-end pointer-events-auto pb-1">
            {isOwner ? (
              <div className="flex items-center justify-between w-full gap-2 mb-1">
                {!isHighlight && (
                  <>
                    <button onClick={() => setShowViewers(true)} className="flex-1 h-10 rounded-full bg-black border border-white/20 flex items-center justify-center hover:bg-white/20 transition-all shadow-xl"><Eye size={18} className="text-white" /></button>
                    <button onClick={() => setIsMentionModalOpen(true)} className="flex-1 h-10 rounded-full bg-black border border-white/20 flex items-center justify-center hover:bg-white/20 transition-all shadow-xl"><AtSign size={18} className="text-white" /></button>
                    <button onClick={() => setIsHighlightModalOpen(true)} className="flex-1 h-10 rounded-full bg-black border border-white/20 flex items-center justify-center hover:bg-white/20 transition-all shadow-xl"><Star size={18} className="text-white" /></button>
                    <button onClick={handleDelete} className="flex-1 h-10 rounded-full bg-black border border-red-500/30 flex items-center justify-center hover:bg-red-500/20 transition-all shadow-xl"><Trash2 size={18} className="text-red-500" /></button>
                  </>
                )}
                {isHighlight && (
                  <button onClick={handleRemoveFromHighlight} className="w-full h-12 rounded-full bg-black border border-red-500/30 flex items-center justify-center gap-2 hover:bg-red-500/20 transition-all shadow-xl"><BookmarkX size={18} className="text-red-500" /><span className="text-[10px] font-black uppercase tracking-widest text-red-500">Rimuovi</span></button>
                )}
              </div>
            ) : !isHighlight && (
              <div className="flex items-center gap-3 w-full mb-1">
                <form onSubmit={handleReply} className="flex-1 flex relative">
                  <Input placeholder={`Rispondi a ${userStories.username}...`} value={replyText} onChange={(e) => setReplyText(e.target.value)} onFocus={() => videoRef.current?.pause()} onBlur={() => videoRef.current?.play()} className="bg-black border border-white/20 rounded-full h-10 px-5 text-[11px] font-bold uppercase tracking-widest text-white placeholder:text-white/70 focus-visible:ring-white/40 shadow-xl" />
                  <AnimatePresence>
                    {(replyText.trim() || justSent) && (
                      <motion.button initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} type="submit" disabled={justSent} className="absolute right-1 top-1 w-8 h-8 bg-white text-black rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform shrink-0 shadow-lg">
                        {justSent ? <Check size={12} className="text-green-600" /> : <Send size={12} className="-rotate-12 ml-0.5" />}
                      </motion.button>
                    )}
                  </AnimatePresence>
                </form>
        ...
        <AnimatePresence>
          {showViewers && (
            <>
              ...
              <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="absolute inset-x-0 z-[61] bg-black border-t border-white/10 rounded-t-[2.5rem] max-h-[60%] flex flex-col pb-10 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]" style={{ touchAction: 'pan-y', bottom: modalBottomOffset }}>
                <div className="w-12 h-1.5 bg-white/10 rounded-full mx-auto mt-4 mb-2 shrink-0" />
                <div className="p-6 border-b border-white/5 flex items-center justify-between"><h3 className="text-lg font-black italic uppercase tracking-tighter">Visualizzazioni</h3><button onClick={() => setShowViewers(false)} className="p-2 text-zinc-500"><X size={24} /></button></div>
                <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                  {views?.length === 0 ? (<div className="text-center py-10 opacity-30"><Eye size={40} className="mx-auto mb-2" /><p className="text-[10px] font-black uppercase tracking-widest">Nessuna visualizzazione</p></div>) : 
                  (views?.map((view: any) => (
                    <button key={view.id} onClick={() => { onClose(); navigate(`/profile/${view.user_id}`); }} className="w-full flex items-center justify-between p-3 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/10 transition-all group">
                      <div className="flex items-center gap-3"><div className="w-10 h-10 bg-black rounded-full overflow-hidden border border-white/10 group-hover:border-white transition-colors">{view.profiles?.avatar_url ? <img src={view.profiles.avatar_url} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center text-zinc-700"><User size={20} /></div>}</div><span className="text-xs font-black italic uppercase text-zinc-300 group-hover:text-white transition-colors">{view.profiles?.username}</span></div>
      ...
      {isOwner && !isHighlight && (
        <AddMentionModal isOpen={isMentionModalOpen} onClose={() => setIsMentionModalOpen(false)} storyId={currentStory.id} storyUrl={currentStory.image_url} existingMentions={currentStory.mentions || []} musicMetadata={currentStory.music_metadata} bottomOffset={modalBottomOffset} />
      )}
    </motion.div>,
    document.body
  );
