...
const StoryViewer = ({ allStories, initialUserIndex, onClose, currentUserId }: StoryViewerProps) => {
  ...
  return createPortal(
    ...
    >
                ...
                {!isHighlight ? (
                  <>
                    <button onClick={() => setShowViewers(true)} className="flex flex-col items-center gap-1 group">
                      ...
                      <span className="text-[7px] font-black uppercase tracking-widest text-white/60">Attività</span>
                    </button>
                    <button onClick={() => setIsMentionModalOpen(true)} className="flex flex-col items-center gap-1 group">
                      ...
                      <span className="text-[7px] font-black uppercase tracking-widest text-white/60">Menziona</span>
                    </button>
                    <button onClick={() => setIsHighlightModalOpen(true)} className="flex flex-col items-center gap-1 group">
                      <Star size={20} className="text-white" />
                      <span className="text-[7px] font-black uppercase tracking-widest text-white/60">Evidenza</span>
                    </button>
                    <button onClick={handleDelete} className="flex flex-col items-center gap-1 group">
                      <Trash2 size={20} className="text-red-500" />
                      <span className="text-[7px] font-black uppercase tracking-widest text-red-500/60">Elimina</span>
                    </button>
                  </>
                ) : (
              ...
              <div className="flex items-center gap-4 w-full">
                <form onSubmit={handleReply} className="flex-1 flex relative">
                  <Input 
                    ...
                    className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-full h-10 px-4 text-[10px] font-bold uppercase tracking-widest text-white placeholder:text-white/50 focus-visible:ring-white/40 shadow-xl" 
                  />
                  <AnimatePresence>
                    {replyText.trim() && (
                      <motion.button 
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <motion.button 
                    ...
                    className={cn(
                      "transition-all", 
                      currentStory.is_liked ? "text-red-500" : "text-white hover:text-zinc-300"
                    )}
                  >
                    <Heart size={22} fill={currentStory.is_liked ? "currentColor" : "none"} />
                  </motion.button>
                  <button onClick={handleShareClick} className="text-white hover:text-zinc-300 transition-all">
                    <Send size={22} className="-rotate-12 mr-0.5" />
                  </button>
                </div>
              </div>
            )}
      ...
      <ShareStoryModal isOpen={isShareModalOpen} onClose={() => setIsShareModalOpen(false)} storyUrl={currentStory.image_url} authorName={userStories.username} bottomOffset={navHeight} />
      {currentUserId && <HighlightModal isOpen={isHighlightModalOpen} onClose={() => setIsHighlightModalOpen(false)} story={currentStory} userId={currentUserId} bottomOffset={navHeight} />}
      {isOwner && !isHighlight && <AddMentionModal isOpen={isMentionModalOpen} onClose={() => setIsMentionModalOpen(false)} storyId={currentStory.id} storyUrl={currentStory.image_url} existingMentions={currentStory.mentions || []} bottomOffset={navHeight} />}
    </motion.div>,
    document.body
  );
};
