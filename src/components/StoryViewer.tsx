  ...
  return createPortal(
    ...
    >
      ...
      <div className="relative w-full h-full md:h-[85vh] md:w-[420px] md:aspect-[9/16] bg-transparent md:bg-black/40 md:rounded-[2.5rem] overflow-hidden flex flex-col shadow-2xl md:border md:border-white/10 z-10">
        ...
        {/* Footer Interaction Area - Sfondo nero solido aggiunto */}
        <div 
          className="absolute bottom-0 left-0 right-0 z-50 select-none bg-black pt-6 pb-[env(safe-area-inset-bottom,12px)] pointer-events-none"
        >
          <div 
            className="px-4 flex w-full max-w-md mx-auto items-center pointer-events-auto" 
          >
            {isOwner ? (
              <div className="flex items-center justify-between w-full gap-2">
                {!isHighlight && (
                  <>
                    <button onClick={() => setShowViewers(true)} className="flex flex-col items-center gap-1 group flex-1">
                      ...
                      <span className="text-[8px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-white">Attività</span>
                    </button>
                    <button onClick={() => setIsMentionModalOpen(true)} className="flex flex-col items-center gap-1 group flex-1">
                      <div className="w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-white/10 transition-all">
                        <AtSign size={16} className="text-white" />
                      </div>
                      <span className="text-[8px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-white">Menziona</span>
                    </button>
                    <button onClick={() => setIsHighlightModalOpen(true)} className="flex flex-col items-center gap-1 group flex-1">
                      <div className="w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-white/10 transition-all">
                        <Star size={16} className="text-white" />
                      </div>
                      <span className="text-[8px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-white">Evidenza</span>
                    </button>
                    <button onClick={handleDelete} className="flex flex-col items-center gap-1 group flex-1">
                      <div className="w-9 h-9 rounded-full bg-white/5 border border-red-500/20 flex items-center justify-center group-hover:bg-red-500/10 transition-all">
                        <Trash2 size={16} className="text-red-500" />
                      </div>
                      <span className="text-[8px] font-black uppercase tracking-widest text-red-500/70 group-hover:text-red-500">Elimina</span>
                    </button>
                  </>
                )}
                {isHighlight && (
                  <button onClick={handleRemoveFromHighlight} className="flex items-center gap-3 group w-full justify-center bg-red-500/10 border border-red-500/20 py-3 rounded-2xl">
              ...
              <div className="flex items-center gap-3 w-full">
                <form onSubmit={handleReply} className="flex-1 flex relative">
                  <Input 
                    ...
                    className="bg-white/5 border border-white/10 rounded-full h-11 px-5 text-[11px] font-bold uppercase tracking-widest text-white placeholder:text-zinc-600 focus-visible:ring-white/20" 
                  />
                  <AnimatePresence>
                    {replyText.trim() && (
                      <motion.button 
                        ...
                        type="submit" 
                        className="absolute right-1.5 top-1.5 w-8 h-8 bg-white text-black rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform shrink-0 shadow-lg"
                      >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <motion.button 
                    ...
                    className={cn(
                      ...
                      currentStory.is_liked ? "bg-red-500 border-red-500 text-white" : "bg-white/5 border-white/10 text-white hover:bg-white/10"
                    )}
                  >
                    <Heart size={18} fill={currentStory.is_liked ? "currentColor" : "none"} />
                  </motion.button>
                  <button onClick={handleShareClick} className="w-11 h-11 bg-white/5 border border-white/10 text-white rounded-full flex items-center justify-center hover:bg-white/10 transition-all">
