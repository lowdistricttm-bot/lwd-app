...
const StoryViewer = ({ allStories, initialUserIndex, onClose, currentUserId }: StoryViewerProps) => {
  ...
  // Offset dinamico per i modal interni basato sulla Safe Area
  const modalBottomOffset = `env(safe-area-inset-bottom)`;
  ...
  return createPortal(
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[9999] bg-black flex items-center justify-center overflow-hidden touch-none h-dvh"
    >
      ...
      <div className="relative w-full h-full md:h-[85vh] md:w-[420px] md:aspect-[9/16] bg-transparent md:bg-black/40 md:rounded-[2.5rem] overflow-hidden flex flex-col shadow-2xl md:border md:border-white/10 z-10">
        ...
        {/* Footer Interaction Area - Trasparente e sovrapposta */}
        <div 
