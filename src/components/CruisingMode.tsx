    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        ...
        supabase.from('vehicles').select('brand, model').eq('user_id', user.id).eq('is_main', true).maybeSingle()
          .then(({ data }) => { if (data) setCarName(`${data.brand} ${data.model}`); });
      }
    });
  }, []);

  useEffect(() => {
    if (isOpen && profile && (!isActive || activeCarovanaId !== carovanaId)) {
      joinChannel(
        carovanaId, 
        ...
        >
          ...
          <AnimatePresence>
            {lastAlert && (
              <motion.div 
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className={cn("absolute inset-0 z-0 opacity-20 blur-[100px] transition-all duration-500", getAlertColor(lastAlert.type))}
              />
            )}
          </AnimatePresence>
          ...
          <div className="relative z-10 p-6 flex items-center justify-between border-b border-white/10 bg-zinc-900/50 backdrop-blur-xl pt-[calc(1.5rem+env(safe-area-inset-top))]">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleDisconnect} className="p-2.5 bg-red-500/10 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition-all border border-red-500/20" title="Disconnetti">
                <Power size={20} />
              </button>
          ...
          <div className="relative z-10 flex-1 p-6 flex flex-col gap-8 overflow-y-auto no-scrollbar">
            <div className="min-h-[120px] flex flex-col justify-center">
              <AnimatePresence mode="wait">
                {lastAlert ? (
                  <motion.div 
                    key="active-alert" initial={{ scale: 0.8, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 1.1, opacity: 0, y: -20 }}
                    className={cn("w-full p-6 rounded-[2.5rem] shadow-2xl flex items-center gap-6 border-2 border-white/20", getAlertColor(lastAlert.type))}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="text-3xl font-black uppercase italic text-white tracking-tight leading-tight">{lastAlert.message}</h4>
                    </div>
                  </motion.div>
                ) : (
                  <motion.div key="idle-state" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center text-center opacity-30 py-8">
                  ...
                  <motion.div key={unit.id} layout className={cn("p-4 rounded-2xl border transition-all duration-300 flex items-center justify-between", unit.isSpeaking ? "bg-orange-600/20 text-orange-500 border-orange-500/30" : "bg-white/5 border-white/5")}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className={cn("text-[9px] font-bold uppercase tracking-widest", unit.isSpeaking ? "text-orange-500/60" : "text-zinc-500")}>{getRoleLabel(unit.role)}</p>
                      </div>
                    </div>
            ...
            <div className="space-y-4 mt-auto">
              ...
              <div className="grid grid-cols-3 gap-3">
                {alerts.map((alert) => (
                  <button key={alert.id} onClick={() => sendAlert(alert.id, alert.msg)} className={cn("flex flex-col items-center justify-center gap-2 p-4 rounded-2xl border border-white/5 transition-all active:scale-95", alert.color, "bg-opacity-20 hover:bg-opacity-30")}>
          ...
          <div className="relative z-10 p-10 pb-[calc(2.5rem+env(safe-area-inset-bottom))] bg-zinc-900/80 backdrop-blur-2xl border-t border-white/10 flex flex-col items-center gap-6">
            <div className="flex flex-col items-center gap-2">
              <div className={cn("px-4 py-1 rounded-full text-[8px] font-black uppercase tracking-[0.3em] italic transition-all", isSpeaking ? "bg-orange-600 text-black animate-pulse" : "bg-zinc-800 text-zinc-500")}>
