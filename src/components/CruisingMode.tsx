...
import { Mic, MicOff, X, Users, Radio, AlertTriangle, Info, Truck, Volume2, ShieldAlert, Loader2 } from 'lucide-react';
...
const CruisingMode = ({ isOpen, onClose, carovanaId, carovanaTitle }: CruisingModeProps) => {
  ...
  const { 
    ...
    carovanaId, 
    user?.user_metadata?.username || user?.email?.split('@')[0] || 'Unit', 
    carName
  );

  useEffect(() => {
    if (isOpen && isLibraryLoaded && !isActive) {
      joinChannel();
    }
    return () => {
      if (isActive) leaveChannel();
    };
  }, [isOpen, isLibraryLoaded, isActive, joinChannel, leaveChannel]);
          ...
          <div className="flex-1 p-6 flex flex-col gap-8 overflow-y-auto no-scrollbar">
            {!isLibraryLoaded ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Inizializzazione Radio...</p>
              </div>
            ) : (
              <>
                <AnimatePresence>
