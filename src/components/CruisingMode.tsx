...
import { Mic, MicOff, X, Users, Radio, AlertTriangle, Info, Truck, Volume2, Zap, ShieldAlert } from 'lucide-react';
import { useCruisingMode } from '@/hooks/use-cruising-mode';
...
const CruisingMode = ({ isOpen, onClose, carovanaId, carovanaTitle }: CruisingModeProps) => {
  ...
  const { 
    isActive, isSpeaking, units, 
    joinChannel, leaveChannel, toggleMic, sendAlert 
  } = useCruisingMode(carovanaId, user?.user_metadata?.username || user?.email?.split('@')[0] || 'Unit', carName);
  ...
  const alerts = [
    ...
    { id: 'stop', label: 'SOSTA', iconthink
The user wants me to continue the previous response exactly where it left off.

Looking at the previous response:
The last file being written was `src/components/CruisingMode.tsx`.
The last line was:
`<dyad-write path="src/components/CruisingMode.tsx" description="Interfaccia Radio CB per la comunicazione in convoglio">`
...
`    { id: 'stop', label: 'SOSTA', icon`

I need to complete the `alerts` array, the rest of the component, and then update `CarovanaDetailModal.tsx` to include the button to trigger this mode.

Plan:
1. Complete `src/components/CruisingMode.tsx`.
2. Update `src/components/CarovanaDetailModal.tsx` to add the "Cruising Mode" button.
3. Provide the chat summary.

Let's finish `src/components/CruisingMode.tsx`:
The `alerts` array needs to be finished.
Then the JSX.
...
Then update `CarovanaDetailModal.tsx`.
...
Wait, I need to make sure I don't miss any imports.
In `CruisingMode.tsx`, I need `Truck` from `lucide-react`.

Let's go.<dyad-write path="src/components/CruisingMode.tsx" description="Completamento interfaccia Radio CB per la comunicazione in convoglio">
"use client";
...
const CruisingMode = ({ isOpen, onClose, carovanaId, carovanaTitle }: CruisingModeProps) => {
  ...
  }, [isOpen, isActive, joinChannel, leaveChannel]);
