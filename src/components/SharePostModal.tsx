...
import { X, Search, User, Send, Loader2 } from 'lucide-react';
        ...
        <>
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-black italic uppercase tracking-tighter">Invia post a...</h3>
              ...
              {isLoading ? (
                ...
                results.map((user) => (
                  <div key={user.id} className="flex items-center justify-between p-3 bg-zinc-900/40 border border-white/5 rounded-2xl">
                    ...
                    >
                      {sendingTo === user.id ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} className="-rotate-12" />}
                    </button>
                  </div>
                ))
              )}
