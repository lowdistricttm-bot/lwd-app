...
const SpotifyPlayer = () => {
  ...
  // Spostiamo il punto di chiusura leggermente più a sinistra per "stringere" sul bordo
  const CLOSED_X = -284;
  ...
  return (
    <div className="fixed bottom-32 left-0 z-[100] pointer-events-none">
      ...
      >
        ...
        {/* Linguetta Ultra-Minimal - Ridotta larghezza a w-6 */}
        <div 
          onClick={() => isOpen ? closePlayer() : openPlayer()}
          className={cn(
            "w-6 h-12 flex items-center justify-center cursor-pointer rounded-r-lg border-y border-r border-white/10 shadow-xl transition-all duration-500",
            isOpen ? "bg-white text-black" : "bg-zinc-900/80 backdrop-blur-md text-white hover:bg-zinc-800"
          )}
        >
          <Music size={14} className={cn(isOpen && "animate-pulse")} />
        </div>
      </motion.div>
    </div>
  );
