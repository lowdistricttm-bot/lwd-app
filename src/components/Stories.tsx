...
const Stories = () => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isSubscriber) { showError("Solo i membri ufficiali possono pubblicare storie."); return; }
  ...
  return (
    <>
      <div 
        className="flex gap-4 overflow-x-auto no-scrollbar items-center px-6 bg-black border-b border-white/10 select-none"
      ...
      >
        {currentUser && (!isSubscriber || myStoriesGroup) && (
          <div className="relative shrink-0">
            ...
            <button 
              ...
              className={cn(
                "w-9 h-9 rounded-full border-2 flex items-center justify-center bg-zinc-900 overflow-hidden transition-all",
            ...
            {!isSubscriber && !myStoriesGroup && (
              <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-white rounded-full flex items-center justify-center border border-black">
                <Plus size={8} className="text-black font-bold" />
              </div>
            )}
          </div>
        )}
        ...
        {otherStories.map((userGroup: any, index: number) => {
          ...
          return (
            <button key={userGroup.user_id} onClick={() => handleStoryClick(actualIndex)} className="shrink-0">
              <div className="w-9 h-9 rounded-full p-[1.5px] bg-gradient-to-tr from-zinc-700 via-zinc-400 to-white">
                <div className="w-full h-full rounded-full border border-black overflow-hidden bg-zinc-900">
                  {userGroup.avatar_url ? <img src={userGroup.avatar_url} className="w-full h-full object-cover" /> : <User size={16} className="m-auto text-zinc-700" />}
                </div>
              </div>
            </button>
          );
      ...
      <AlertDialog open={isAuthModalOpen} onOpenChange={setIsAuthModalOpen}>
        <AlertDialogContent className="bg-zinc-950 border-white/10 rounded-none">
          ...
          <AlertDialogFooter className="flex flex-col gap-2 sm:flex-col">
            <AlertDialogAction onClick={() => navigate('/login')} className="rounded-none bg-white text-black hover:bg-zinc-200 font-black uppercase italic text-[10px] w-full h-12 transition-all">Accedi Ora <ArrowRight size={14} className="ml-2" /></AlertDialogAction>
            <AlertDialogCancel className="rounded-none border-white/10 text-white hover:bg-white/5 font-black uppercase italic text-[10px] w-full h-12 mt-0 transition-all">Chiudi</AlertDialogCancel>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
