...
const CarovanaDetailModal = ({ isOpen, onClose, carovana, currentUserId, onEdit }: CarovanaDetailModalProps) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div 
            ...
            onClick={onClose} className="fixed inset-0 bg-black/80 z-[250] touch-none"
            data-no-swipe="true"
          />
          <motion.div 
            ...
            <div className="max-w-2xl mx-auto space-y-8 pb-10">
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <span className="bg-white text-black text-[8px] font-black uppercase px-2 py-1 italic rounded-lg shadow-lg">RUN TO THE SHOW</span>
                    {carovana.privacy === 'private' && (
                      <span className="bg-zinc-800 text-zinc-400 text-[8px] font-black uppercase px-2 py-1 italic rounded-lg flex items-center gap-1"><Lock size={10} /> PRIVATA</span>
                    )}
                  </div>
                  <h3 className="text-3xl font-black italic uppercase tracking-tighter leading-tight">{carovana.title}</h3>
                </div>
                <div className="flex gap-2">
                  {isCreator && (
                    <>
                      <button onClick={() => onEdit?.(carovana)} className="p-2 bg-white/5 text-zinc-400 rounded-full hover:bg-white hover:text-black transition-all"><Edit3 size={20} /></button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-white/5 border border-white/10 p-5 rounded-[2rem]">
                  ...
                  <div className="flex items-center gap-3 text-white mb-2"><MapPin size={16} className="text-zinc-400" /><span className="text-sm font-black uppercase italic">{carovana.start_location}</span></div>
                  <div className="flex items-center gap-3 text-white"><Clock size={16} className="text-zinc-400" /><span className="text-sm font-black uppercase italic">{format(new Date(carovana.start_time), 'dd MMM - HH:mm', { locale: it }).toUpperCase()}</span></div>
                </div>
                <div className="bg-white/5 border border-white/10 p-5 rounded-[2rem]">
                  <p className="text-[8px] font-black uppercase text-zinc-500 tracking-[0.3em] italic mb-2">Organizzatore</p>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-black border border-white/10">{carovana.profiles?.avatar_url ? <img src={carovana.profiles.avatar_url} className="w-full h-full object-cover" /> : <Users size={20} className="m-auto h-full text-zinc-700" />}</div>
              ...
              {carovana.carovane_tappe && carovana.carovane_tappe.length > 0 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-white/5 border border-white/5 p-6 rounded-[2.5rem] space-y-6 relative">
                    ...
                    {carovana.carovane_tappe.map((stop) => (
                      <div key={stop.id} className="flex items-start gap-6 relative z-10">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start">
                            ...
                            {stop.arrival_time && <span className="text-[9px] font-black uppercase text-zinc-500 bg-white/5 px-2 py-0.5 rounded-md">{format(new Date(stop.arrival_time), 'HH:mm')}</span>}
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {participants.map((p, i) => (
                    <div key={i} className="bg-white/5 border border-white/5 p-4 rounded-2xl flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-black border border-white/10">{p.profiles?.avatar_url ? <img src={p.profiles.avatar_url} className="w-full h-full object-cover" /> : <Users size={16} className="m-auto h-full text-zinc-700" />}</div>
                      <div className="min-w-0"><p className="text-[10px] font-black uppercase italic text-white truncate">@{p.profiles?.username}</p><p className="text-[8px] font-bold uppercase text-zinc-500 truncate">{p.vehicles?.brand} {p.vehicles?.model}</p></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-6 flex flex-col gap-4">
                <Button 
                  ...
                  className={cn(
                    ...
                    carovana.is_joined ? "bg-zinc-800 text-white border-white/10 hover:bg-red-600" : "bg-white text-black border-white hover:bg-zinc-200"
                  )}
                >
                  {toggleJoin.isPending ? <Loader2 className="animate-spin" size={18} /> : carovana.is_joined ? 'ABBANDONA CAROVANA' : 'UNISCITI ALLA CAROVANA'}
                </Button>
                <div className="flex gap-3">
                  <Button onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(carovana.start_location)}`, '_blank')} variant="outline" className="flex-1 h-14 rounded-full font-black uppercase italic text-[9px] tracking-widest border-white/10 text-white hover:bg-white/5 flex items-center justify-center gap-2"><Navigation size={16} /> VAI ALLA PARTENZA</Button>
                  <Button onClick={handleShare} variant="outline" className="flex-1 h-14 rounded-full font-black uppercase italic text-[9px] tracking-widest border-white/10 text-white hover:bg-white/5 flex items-center justify-center gap-2"><Share2 size={16} /> CONDIVIDI</Button>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <CruisingMode isOpen={isCruisingOpen} onClose={() => setIsCruisingOpen(false)} carovanaId={carovana.id} carovanaTitle={carovana.title} />
    </AnimatePresence>
  );
};
